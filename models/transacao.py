...
from models.models import Transacao, ProdutoTransacao, Produto
...
def create_transacao_compra(db: Session, cliente_id: str, produtos: list):
...
def create_transacao_resgate(db: Session, cliente_id: str, produtos: list[Produto]):
    
    valor_moeda_cento = sum([p.preco_venda * q for p,q in produtos])
    valor_moeda_cento_final = 0.0
    valor_pontos = sum([(-1)*p.pontos_resgate * q for p,q in produtos])
